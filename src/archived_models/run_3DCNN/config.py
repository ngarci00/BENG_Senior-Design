...
# index_json_path = os.path.join(json_dir, "index_rec.json") #Path to index JSON file for rectangle annotations
index_json_path = os.path.join(json_dir, "index_poly.json") #Path to index JSON file for polygon annotations

# splits_json_path = os.path.join(json_dir, "splits_rec.json") #Path to splits JSON file for train/val splits (all videos)
# splits_json_path = os.path.join(json_dir, "splits_rec_20.json") #<-- Temorary, smaller splits for quick testing (20)
splits_json_path = os.path.join(json_dir, "splits_poly_50.json") #Path to splits JSON file for train/val splits
...
resize_hw = (320,320) #HxW to resize frames for model input. This is a common value for 3D CNNs (112x112)
