...
def _save_roc_curve(y_true: List[int], y_prob: List[float], out_dir:str, fold: int) -> None:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, f"fold_{fold}_roc.png"))
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, f"fold_{fold}_pr.png"))
    plt.close()

@torch.no_grad() #No need to compute gradients during evaluation
def _infer_fold(fold: int, device: int, eval_clips_per_video: int, batch_size: int, num_workers: int) -> Tuple[List[int], Dict[str, float]]:
    ...
    if len(set(y_true)) == 2: #Only plot ROC curve if we have both classes present
        fpr, tpr = roc_curve(y_true, y_prob) #False positive rate, true positive rate for ROC curve
...
def main():
    ...
    report_dirt =os.path.join(runs_path, "reports")
    _ensure_dir(report_dirt)
    ...
    for fold in folds:
        ...
        _write_csv(os.path.join(report_dirt, f"fold_{fold}_results.csv"), rows) #Save results for this fold
        ...
        cm = confusion_matrix(y_true, y_pred, labels=[0,1])
        _save_confusion_matrix(cm, os.path.join(report_dirt, f"fold_{fold}_confusion_matrix.png"), title=f"Fold {fold} Confusion Matrix")
        #Save ROC curve plot for this fold
        _save_roc_curve(y_true, y_prob, report_dirt, fold)
        ...
        _write_csv(os.path.join(report_dirt, "all_folds_metrics.csv"), all_rows) #Save combined results for all folds
        _write_csv(os.path.join(report_dirt, "all_folds_summary.csv"), _add_mean_std(all_metrics)) #Save summary metrics with mean and std across folds

        with open(os.path.join(report_dirt, "all_folds_metrics.json"), "w") as f:
            json.dump(all_metrics, f, indent=2) #Save metrics in JSON format for easier parsing later (:

        print(f"Evaluation complete. Results saved to {report_dirt} !!!")
